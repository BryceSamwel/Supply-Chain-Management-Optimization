...
\sum_{i=1}^{4}\sum_{j=1}^{5} x_{i,j} \leq R_1,\\ 
...
model <- MIPModel() %>%
  ...
  set_objective(sum_expr(C[j]*x[i,j],
                         i=1:NSubregions,
                         j=1:NBuildingTypes), "max")%>%
  
  add_constraint(sum_expr(x[i,j],i=1:4,j=1:5) <= R[1])%>%
  add_constraint(sum_expr(x[i,j],i=5:7,j=1:5) <= R[2])%>%
  add_constraint(sum_expr(x[i,j],i=8:12,j=1:5) <= R[3])%>%
  add_constraint(sum_expr(x[13,j],j=1:5) <= R[4])%>%
  add_constraint(sum_expr(x[i,j],i=14:15,j=1:5) <= R[5])%>%
